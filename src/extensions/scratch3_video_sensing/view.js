...
class VideoMotionView {
     ...
     * @type {object}
     */
    static _videoMotionViewComponentsTmp = {
        A2: 0,
        A1B2: 0,
        B1: 0,
        C2: 0,
    ...
    constructor (motion, output = VideoMotionView.OUTPUT.XYT) {
        /**
        ...
        canvas.width = VideoMotionView.WIDTH;
        canvas.height = VideoMotionView.HEIGHT;
         ...
         * @type {Uint32Array}
         */
        this.buffer = new Uint32Array(VideoMotionView.WIDTH * VideoMotionView.HEIGHT);
    }
     ...
     */
    static get OUTPUT () {
        return VideoMotionView._OUTPUT;
    }
    ...
    _eachAddress (xStart, yStart, xStop, yStop, fn) {
        for (let i = yStart; i < yStop; i++) {
            for (let j = xStart; j < xStop; j++) {
                const address = (i * VideoMotionView.WIDTH) + j;
                fn(address, j, i);
            }
        }
    }
    ...
    _grads (address) {
        ...
        const gradY = (curr[address - VideoMotionView.WIDTH] & 0xff) - (curr[address + VideoMotionView.WIDTH] & 0xff);
    ...
    _components (eachAddress) {
        ...
        VideoMotionView._videoMotionViewComponentsTmp.A2 = A2;
        VideoMotionView._videoMotionViewComponentsTmp.A1B2 = A1B2;
        VideoMotionView._videoMotionViewComponentsTmp.B1 = B1;
        VideoMotionView._videoMotionViewComponentsTmp.C2 = C2;
        VideoMotionView._videoMotionViewComponentsTmp.C1 = C1;
        return VideoMotionView._videoMotionViewComponentsTmp;
    }
    ...
    draw () {
        ...
        if (this.output === VideoMotionView.OUTPUT.INPUT) {
            const {curr} = this.motion;
            this._eachAddress(1, 1, VideoMotionView.WIDTH - 1, VideoMotionView.HEIGHT - 1, address => {
                buffer[address] = curr[address];
            });
        }
        if (this.output === VideoMotionView.OUTPUT.XYT) {
            this._eachAddress(1, 1, VideoMotionView.WIDTH - 1, VideoMotionView.HEIGHT - 1, address => {
                const {gradX, gradY, gradT} = this._grads(address);
        ...
        if (this.output === VideoMotionView.OUTPUT.XYT_CELL) {
            const winStep = (VideoMotionView.WINSIZE * 2) + 1;
            const wmax = VideoMotionView.WIDTH - VideoMotionView.WINSIZE - 1;
            const hmax = VideoMotionView.HEIGHT - VideoMotionView.WINSIZE - 1;

            this._eachCell(VideoMotionView.WINSIZE + 1, VideoMotionView.WINSIZE + 1, wmax, hmax, winStep, winStep,
                eachAddress => {
                    let C1 = 0;
                    let C2 = 0;
                    let n = 0;
        ...
        } else if (this.output === VideoMotionView.OUTPUT.T) {
            this._eachAddress(1, 1, VideoMotionView.WIDTH - 1, VideoMotionView.HEIGHT - 1, address => {
                const {gradT} = this._grads(address);
                buffer[address] = (0xff << 24) + ((gradT + 0xff) / 2 << 16);
            });
        }
        if (this.output === VideoMotionView.OUTPUT.T_CELL) {
            const winStep = (VideoMotionView.WINSIZE * 2) + 1;
            const wmax = VideoMotionView.WIDTH - VideoMotionView.WINSIZE - 1;
            const hmax = VideoMotionView.HEIGHT - VideoMotionView.WINSIZE - 1;
        ...
        } else if (this.output === VideoMotionView.OUTPUT.C) {
            this._eachAddress(1, 1, VideoMotionView.WIDTH - 1, VideoMotionView.HEIGHT - 1, address => {
                const {gradX, gradY, gradT} = this._grads(address);
                buffer[address] =
                    (0xff << 24) +
        ...
        if (this.output === VideoMotionView.OUTPUT.C_CELL) {
            const winStep = (VideoMotionView.WINSIZE * 2) + 1;
            const wmax = VideoMotionView.WIDTH - VideoMotionView.WINSIZE - 1;
            const hmax = VideoMotionView.HEIGHT - VideoMotionView.WINSIZE - 1;
        ...
        } else if (this.output === VideoMotionView.OUTPUT.AB) {
            this._eachAddress(1, 1, VideoMotionView.WIDTH - 1, VideoMotionView.HEIGHT - 1, address => {
                const {gradX, gradY} = this._grads(address);
                buffer[address] =
                    (0xff << 24) +
        ...
        if (this.output === VideoMotionView.OUTPUT.AB_CELL) {
            const winStep = (VideoMotionView.WINSIZE * 2) + 1;
            const wmax = VideoMotionView.WIDTH - VideoMotionView.WINSIZE - 1;
            const hmax = VideoMotionView.HEIGHT - VideoMotionView.WINSIZE - 1;
        ...
        } else if (this.output === VideoMotionView.OUTPUT.UV) {
            const winStep = (VideoMotionView.WINSIZE * 2) + 1;
            ...
            this._eachAddress(1, 1, VideoMotionView.WIDTH - 1, VideoMotionView.HEIGHT - 1, address => {
                ...
                const amount = VideoMotionView.AMOUNT_SCALE * hypot;

                buffer[address] =
                    (0xff << 24) +
                    (inRange && amount > VideoMotionView.THRESHOLD ?
                        (((((v / winStep) + 1) / 2 * 0xff) << 8) & 0xff00) +
                        (((((u / winStep) + 1) / 2 * 0xff) << 0) & 0xff) :
                        0x8080
                    );
            });
        } else if (this.output === VideoMotionView.OUTPUT.UV_CELL) {
            const winStep = (VideoMotionView.WINSIZE * 2) + 1;
            const wmax = VideoMotionView.WIDTH - VideoMotionView.WINSIZE - 1;
            const hmax = VideoMotionView.HEIGHT - VideoMotionView.WINSIZE - 1;
        ...
        const data = new ImageData(new Uint8ClampedArray(this.buffer.buffer),
            VideoMotionView.WIDTH, VideoMotionView.HEIGHT);
