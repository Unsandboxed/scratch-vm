...
E.serializeVariables = function (variables, runtime, {
    ...
    for (const varId in variables) {
        ...
        if (v.type === Variable.LIST_TYPE) {
            ...
            if (includeCustomTypes && runtime && typeof runtime.serializeCustomTypeValueDeep === 'function') {
                listValue = runtime.serializeCustomTypeValueDeep(listValue);
            } else if (Array.isArray(listValue)) {
                listValue = listValue.map(item => normalizeSb3Value(item));
            }
            obj.lists[varId] = includeListLockState ? [v.name, listValue, v.locked] : [v.name, listValue];
            continue;
        }
        ...
        if (includeCustomTypes && runtime && typeof runtime.serializeCustomTypeValueDeep === 'function') {
            ...
            scalarValue = normalizeSb3Value(scalarValue);
        }
...
E.serializeTarget = function (target, extensions, runtime, {
    ...
    obj.frames = E.serializeFrames(target.frames || {}, {
        includeLockState: includeFrameLockState
    });
...
E.serializeMonitors = function (monitors, runtime, extensions, {
    ...
    const serializeMonitorParams = params => {
        if (includeCustomTypes && runtime && typeof runtime.serializeCustomTypeValueDeep === 'function') {
            return runtime.serializeCustomTypeValueDeep(params);
        }
        const normalized = Object.create(null);
        for (const [key, value] of Object.entries(params || {})) {
            normalized[key] = normalizeSb3Value(value);
        }
        return normalized;
    };
    ...
    return monitors.valueSeq()
        ...
        .map(monitorData => {
            const serializedMonitor = {
                ...
                params: serializeMonitorParams(monitorData.params),
...
E.deserialize = async function (json, runtime, zip, isSingleSprite) {
    ...
    if (Array.isArray(projectJSON.extensions)) {
        extensions.declaredExtensionIDs = new Set(projectJSON.extensions);
    ...
    if (projectJSON.meta && projectJSON.meta.origin) {
        // eslint-disable-next-line require-atomic-updates
        runtime.origin = projectJSON.meta.origin;
    } else {
        // eslint-disable-next-line require-atomic-updates
        runtime.origin = null;
    }
    ...
    if (projectJSON.extensionURLs) {
        for (const [id, url] of Object.entries(projectJSON.extensionURLs)) {
    ...
    let fontPromise;
    if (projectJSON.customFonts) {
        fontPromise = runtime.fontManager.deserialize(projectJSON.customFonts, zip, isSingleSprite);
    } else {
    ...
    const targetObjects = ((isSingleSprite ? [projectJSON] : projectJSON.targets) || [])
        .map((t, i) => Object.assign(t, {targetPaneOrder: i}))
        .sort((a, b) => a.layerOrder - b.layerOrder);

    const monitorObjects = projectJSON.monitors || [];
    ...
    return fontPromise.then(() => targetObjects.map(target => E.parseScratchAssets(target, runtime, zip)))
        ...
        .then(targets => {
            ...
            if (Object.prototype.hasOwnProperty.call(projectJSON, 'extensionStorage')) {
                const decodedExtensionStorage =
                    customTypesEnabled && runtime && typeof runtime.deserializeCustomTypeValueDeep === 'function' ?
                        runtime.deserializeCustomTypeValueDeep(projectJSON.extensionStorage) :
                        projectJSON.extensionStorage;
                runtime.store.unsafe$setExtensionStorage(Object.assign(Object.create(null), decodedExtensionStorage || {}));
            }
            if (Object.prototype.hasOwnProperty.call(projectJSON, 'projectStorage')) {
                const decodedProjectStorage =
                    customTypesEnabled && runtime && typeof runtime.deserializeCustomTypeValueDeep === 'function' ?
                        runtime.deserializeCustomTypeValueDeep(projectJSON.projectStorage) :
                        projectJSON.projectStorage;
