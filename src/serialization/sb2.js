...
const specMapTools = specMap[0];
delete specMap[0]; // HACK: Clean up the specMap.
...
E.WORKSPACE_X_SCALE = 1.5;
E.WORKSPACE_Y_SCALE = 2.2;
...
E.SCRATCHX_OPCODE_SEPARATOR = /\u001f|\./;

/**
 * @param {string} opcode
 * @returns {boolean}
 */
E.isPossiblyScratchXBlock = opcode => E.SCRATCHX_OPCODE_SEPARATOR.test(opcode);

/**
 * @param {string} opcode
 * @returns {string}
 */
E.mapScratchXOpcode = opcode => {
    const [extensionName, extensionMethod] = opcode.split(E.SCRATCHX_OPCODE_SEPARATOR);
...
E.mapScratchXBlock = block => {
    ...
    return {
        opcode: E.mapScratchXOpcode(opcode),
        argMap: args
    };
};
 ...
 */
E.parseProcedureArgMap = function (procCode) {
    const argMap = [
 ...
 */
E.parseProcedureArgIds = function (procCode) {
    return E.parseProcedureArgMap(procCode)
 ...
 */
E.flatten = function (blocks) {
    let finalBlocks = [];
    for (let i = 0; i < blocks.length; i++) {
        const block = blocks[i];
        finalBlocks.push(block);
        if (block.children) {
            finalBlocks = finalBlocks.concat(E.flatten(block.children));
        }
 ...
 */
E.parseBlockList = function (blockList, addBroadcastMsg, getVariableId, extensions, parseState, comments,
    commentIndex) {
    const resultingList = [];
    ...
    for (let i = 0; i < blockList.length; i++) {
        ...
        const parsedBlockAndComments = E.parseBlock(block, addBroadcastMsg, getVariableId,
 ...
 */
E.parseScripts = function (scripts, blocks, addBroadcastMsg, getVariableId, extensions, comments) {
    ...
    for (let i = 0; i < scripts.length; i++) {
        ...
        const parseState = {};
        const [parsedBlockList, newCommentIndex] =
            E.parseBlockList(blockList, addBroadcastMsg, getVariableId, extensions,
                parseState, comments, scriptIndexForComment);
        scriptIndexForComment = newCommentIndex;
        if (parsedBlockList[0]) {
            parsedBlockList[0].x = scriptX * E.WORKSPACE_X_SCALE;
            parsedBlockList[0].y = scriptY * E.WORKSPACE_Y_SCALE;
            parsedBlockList[0].topLevel = true;
            parsedBlockList[0].parent = null;
        }
        // Flatten children and create add the blocks.
        const convertedBlocks = E.flatten(parsedBlockList);
 ...
 */
E.generateVariableIdGetter = (function () {
...
E.globalBroadcastMsgStateGenerator = (function () {
...
E.parseMonitorObject = (object, runtime, targets, extensions) => {
    ...
    if (extID === 'videoSensing') {
        return;
    } else if (E.CORE_EXTENSIONS.indexOf(extID) === -1 && extID !== '' &&
    ...
    const getVariableId = E.generateVariableIdGetter(target.id, false);
    // eslint-disable-next-line no-use-before-define
    const [block, _] = E.parseBlock(
    ...
    if (existingMonitorBlock) {
        ...
        const newBlocks = E.flatten([block]);
 ...
 *   objects.
 */
E.parseScratchAssets = function (object, runtime, topLevel, zip) {
    ...
    if (object.children) {
        for (let m = 0; m < object.children.length; m++) {
            childrenAssets.push(E.parseScratchAssets(object.children[m], runtime, false, zip));
        }
    }
 ...
 */
E.parseScratchObject = function (object, runtime, extensions, topLevel, zip, assets) {
    if (!Object.prototype.hasOwnProperty.call(object, 'objName')) {
    ...
    const getVariableId = E.generateVariableIdGetter(target.id, topLevel);

    const globalBroadcastMsgObj = E.globalBroadcastMsgStateGenerator(topLevel);
    ...
    if (Object.prototype.hasOwnProperty.call(object, 'scriptComments')) {
        const comments = object.scriptComments.map(commentDesc => {
            ...
            const newComment = new Comment(
                ...
                isBlockComment ? null : commentX * E.WORKSPACE_X_SCALE,
                isBlockComment ? null : commentY * E.WORKSPACE_Y_SCALE,
                commentWidth * E.WORKSPACE_X_SCALE,
                commentHeight * E.WORKSPACE_Y_SCALE,
                !commentFullSize
            );
            if (isBlockComment) {
    ...
    if (Object.prototype.hasOwnProperty.call(object, 'scripts')) {
        E.parseScripts(object.scripts, blocks, addBroadcastMsg, getVariableId, extensions, blockComments);
    }
    ...
    if (object.children) {
        for (let m = 0; m < object.children.length; m++) {
            childrenPromises.push(
                E.parseScratchObject(object.children[m], runtime, extensions, false, zip, assets.children[m])
            );
        }
    }
    ...
    return Promise.all(
        ...
        Promise.all(
            ...
            for (let n = 0; n < deferredMonitors.length; n++) {
                E.parseMonitorObject(deferredMonitors[n], runtime, targets, extensions);
            }
            return targets;
        })
    );
...
E.reorderParsedTargets = function (targets) {
 ...
 */
E.sb2import = function (json, runtime, optForceSprite, zip) {
    const extensions = {
        extensionIDs: new Set(),
        extensionURLs: new Map()
    };
    return Promise.resolve(E.parseScratchAssets(json, runtime, !optForceSprite, zip))
        // Force this promise to wait for the next loop in the js tick. Let
        // storage have some time to send off asset requests.
        .then(assets => Promise.resolve(assets))
        .then(assets => (
            E.parseScratchObject(json, runtime, extensions, !optForceSprite, zip, assets)
        ))
        .then(E.reorderParsedTargets)
        .then(targets => ({
            targets,
            extensions
        }));
 ...
 */
E.specMapBlock = function (block) {
    const opcode = block[0];
    const mapped = opcode && specMap[opcode];
    if (!mapped) {
        if (opcode && E.isPossiblyScratchXBlock(opcode)) {
            return E.mapScratchXBlock(block);
        }
 ...
 */
E.parseBlock = function (sb2block, addBroadcastMsg, getVariableId, extensions, parseState, comments, commentIndex) {
    const commentsForParsedBlock = (comments && typeof commentIndex === 'number' && !isNaN(commentIndex)) ?
        comments[commentIndex] : null;
    const blockMetadata = E.specMapBlock(sb2block);
    if (!blockMetadata) {
    ...
    if (oldOpcode === 'call') {
        blockMetadata.argMap = E.parseProcedureArgMap(sb2block[1]);
    }
        ...
        if (expectedArg.type === 'input') {
            ...
            if (typeof providedArg === 'object' && providedArg) {
                ...
                if (typeof providedArg[0] === 'object' && providedArg[0]) {
                    ...
                    [innerBlocks, commentIndex] = E.parseBlockList(providedArg, addBroadcastMsg, getVariableId,
                        extensions, parseState, comments, commentIndex);
                } else {
                    // Single block occupies the input.
                    const parsedBlockDesc = E.parseBlock(providedArg, addBroadcastMsg, getVariableId, extensions,
    ...
    if (oldOpcode === 'stopScripts') {
        ...
        activeBlock.children = [{
            ...
            mutation: {
                ...
                argumentids: JSON.stringify(E.parseProcedureArgIds(procData[0])),
        ...
        activeBlock.mutation = {
            ...
            children: [],
            proccode: sb2block[1],
            argumentids: JSON.stringify(E.parseProcedureArgIds(sb2block[1]))
        };
...
module.exports.deserialize = E.sb2import;
