...
module.exports = function (compilerData, {
    ...
    compilerData.registerBlock([
        ...
        function (jsg, block) {
            ...
            if (
                ...
                return `(${
                    jsg.descendInput(left.toType(InputType.NUMBER))
                } > ${jsg.descendInput(right.toType(InputType.NUMBER_OR_NAN))})`;
            }
            ...
            if (
                ...
                return `!(${
                    jsg.descendInput(left.toType(InputType.NUMBER_OR_NAN))
                } <= ${jsg.descendInput(right.toType(InputType.NUMBER))})`;
            }
    ...
    compilerData.registerBlock('operator_contains', function (stg, block) {
        ...
        const node = block.inputs;
        return `(${
            jsg.descendInput(node.string)
        }.toLowerCase().indexOf(${jsg.descendInput(node.contains)}.toLowerCase()) !== -1)`;
    }, {
        input: true,
        type: InputType.BOOLEAN
    });
